...
def render_debug_page():
    ...
    with t1:
        ...
        st.markdown("""
        **Semi-Supervised Learning** combines labeled data (from constants.py) with 
        self-learned patterns. The system improves over time by learning from high-confidence predictions.
        """)
        
        # Get learning stats
        ssl_stats = ml_utils.get_semi_supervised_stats()
        
        # Status indicator
        if ssl_stats.get("learning_enabled", False):
            status_color = "#00C853"
            status_text = "LEARNING ENABLED"
        else:
            status_color = "#FF8F00"
            status_text = "LEARNING DISABLED"
        
        st.markdown(f"""
        <div style='display: flex; align-items: center; gap: 1rem; margin-bottom: 1rem;'>
            <span style='background: {status_color}; color: #0d1117; padding: 4px 12px; 
        ...
        # Stats metrics
        ssl1, ssl2, ssl3, ssl4 = st.columns(4)
        with ssl1:
            st.metric("Patterns Learned", ssl_stats.get("total_patterns_learned", 0),
                     help="Total new skill patterns learned from analysis")
        with ssl2:
            st.metric("Skills Enhanced", ssl_stats.get("skills_enhanced", 0),
                     help="Number of unique skills with learned patterns")
        with ssl3:
            st.metric("Synonyms", ssl_stats.get("skill_synonyms_count", 0),
                     help="New keyword variations learned")
        with ssl4:
            st.metric("Context Patterns", ssl_stats.get("context_patterns_count", 0),
                     help="Contextual associations learned")
        
        # Technical explanation
        with st.expander("7. Semi-Supervised Learning - Label Propagation", expanded=False):
            st.markdown("""
            **Purpose:** Automatically expand the skill database by learning from high-confidence predictions.
            
            **Algorithm:** Label Propagation with TF-IDF Similarity
            
            | Parameter | Value | Why |
            |-----------|-------|-----|
            | Confidence Threshold | 85% | Only learn from reliable predictions |
            | Similarity Threshold | 70% | Propagate labels to similar text |
            | Max Patterns/Skill | 20 | Prevent noise accumulation |
            
            **How it works:**
            1. Random Forest predicts skills with confidence scores
            2. High-confidence predictions (>85%) are used as "seeds"
            3. Similar text chunks inherit the skill label (Label Propagation)
            4. New patterns are saved to `learned_patterns.json`
            
            **Persistence:** Learned patterns are stored permanently and loaded at startup.
            The more you use the system, the smarter it becomes.
            """)
        
        # Top enhanced skills
        if ssl_stats.get("top_enhanced_skills"):
            with st.expander(f"Top Enhanced Skills ({len(ssl_stats['top_enhanced_skills'])})", expanded=False):
                st.caption("Skills that have learned the most new patterns")
                for skill in ssl_stats["top_enhanced_skills"]:
                    st.markdown(f"• **{skill}**")
        
        # Last updated
        if ssl_stats.get("last_updated"):
            st.caption(f"Last learning update: {ssl_stats['last_updated']}")
        
        # Reset button
        ssl_act1, ssl_act2, ssl_act3 = st.columns(3)
        with ssl_act1:
            if st.button("Reset Learned Patterns", use_container_width=True, type="secondary"):
                if ml_utils.reset_semi_supervised_learning():
                    st.success("All learned patterns have been reset!")
                    st.rerun()
                else:
                    st.warning("Semi-supervised module not available")
    ...
    with t6:
        ...
        st.info("Live view of the raw data currently generated by the CV Builder Wizard.")
...
def render_landing_page():
    ...
    with col1:
        with st.container(border=True):
            st.markdown(card_style.format("CV Builder", "Build a professional CV with our AI Wizard. Get real-time suggestions to match your dream job profile."), unsafe_allow_html=True)
